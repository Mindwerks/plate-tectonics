...
class Rectangle {
  public:
    Rectangle(const WorldDimension& worldDimension, uint32_t left, uint32_t right, uint32_t top,
              uint32_t bottom)
        : _worldDimension(worldDimension), _left(left), _right(right), _top(top),
          _bottom(bottom) {};

    Rectangle(const Rectangle& original)
        : _worldDimension(original._worldDimension), _left(original._left), _right(original._right),
        ...
        if (this != &original) {
            ...
            if (&_worldDimension != &original._worldDimension) {
                throw std::invalid_argument(
                    "Cannot assign rectangles with different world dimensions");
            }
    ...
    bool contains(uint32_t x, uint32_t y) const {
        ...
        if (cleanX < getLeft())
            cleanX += _worldDimension.getWidth();
        if (cleanY < getTop())
            cleanY += _worldDimension.getHeight();
