...
class lithosphere {
  ...
  private:
    ...
    void updateHeightAndPlateIndexMaps(const uint32_t& map_area, uint32_t& oceanic_collisions,
    ...
    void resolveJuxtapositions(const uint32_t& i, const uint32_t& j, const uint32_t& k,
                               const uint32_t& x_mod, const uint32_t& y_mod, const float*& this_map,
                               const uint32_t*& this_age, uint32_t& continental_collisions);
    ...
    class plateCollision {
      public:
        plateCollision(uint32_t _index, uint32_t x, uint32_t y, float z) noexcept
