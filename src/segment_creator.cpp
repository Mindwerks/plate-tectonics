...
uint32_t MySegmentCreator::calcDirection(uint32_t x, uint32_t y, const uint32_t origin_index,
    ...
    uint32_t canGoDown =
        y < _bounds.height() - 1 && map[origin_index + _bounds.width()] >= CONT_BASE;
...
void MySegmentCreator::scanSpans(const uint32_t line, uint32_t& start, uint32_t& end,
                                 std::vector<uint32_t>* spans_todo,
                                 std::vector<uint32_t>* spans_done) const {
    do // Find an unscanned span on this line.
    {
        ...
        for (uint32_t j = 0; j < spans_done[line].size(); j += 2) {
...
ContinentId MySegmentCreator::createSegment(uint32_t x, uint32_t y) const noexcept {
    ...
    do {
        ...
        for (uint32_t line = 0; line < bounds_height; ++line) {
            ...
            const uint32_t row_above =
                ((line - 1) & -(line > 0)) | ((bounds_height - 1) & -(line == 0));
            ...
            while (end < bounds_width - 1 && _segments->id(line_here + end + 1) > ID &&
            ...
            if (bounds_width == _worldDimension.getWidth() && end == bounds_width - 1 &&
                _segments->id(line_here + 0) > ID && map[line_here + 0] >= CONT_BASE) {
            ...
            if (start < pData->getLeft())
                pData->setLeft(start);
            if (end > pData->getRight())
                pData->setRight(end);

            if (line > 0 || bounds_height == _worldDimension.getHeight()) {
                for (uint32_t j = start; j <= end; ++j)
                    if (_segments->id(line_above + j) > ID && map[line_above + j] >= CONT_BASE) {
                        uint32_t a = j;
                        _segments->setId(line_above + a, ID);

                        // Count volume of pixel...

                        while (++j < bounds_width && _segments->id(line_above + j) > ID &&
            ...
            if (line < bounds_height - 1 || bounds_height == _worldDimension.getHeight()) {
                for (uint32_t j = start; j <= end; ++j)
                    if (_segments->id(line_below + j) > ID && map[line_below + j] >= CONT_BASE) {
                        uint32_t a = j;
                        _segments->setId(line_below + a, ID);

                        // Count volume of pixel...

                        while (++j < bounds_width && _segments->id(line_below + j) > ID &&
