...
void createSlowNoise(float* map, const WorldDimension& tmpDim, SimpleRandom randsource) {
    ...
    for (uint32_t y = 0; y < height; y++) {
        for (uint32_t x = 0; x < width; x++) {
            ...
            float v = scaled_octave_noise_4d(4.0f, persistence, 0.25f, 0.0f, 1.0f,
...
void createNoise(float* tmp, const WorldDimension& tmpDim, SimpleRandom randsource,
                 bool useSimplex) {
    try {
        if (useSimplex) {
            simplexnoise(randsource.next(), tmp, tmpDim.getWidth(), tmpDim.getHeight(),
                         SQRDMD_ROUGHNESS);
        } else {
            ...
            for (uint32_t y = 0; y < tmpDim.getHeight(); y++) {
                memcpy(&squareTmp[y * side], &tmp[y * tmpDim.getWidth()],
                       sizeof(float) * tmpDim.getWidth());
            }
            ...
            for (uint32_t y = 0; y < tmpDim.getHeight(); y++) {
                for (uint32_t x = tmpDim.getWidth(); x < side; x++) {
                    // we simply put it as a mix between the east and west border (they should be
                    // fairly similar because it is a toroidal world)
                    squareTmp[y * side + x] =
                        (squareTmp[y * side + 0] + squareTmp[y * side + (tmpDim.getWidth() - 1)]) /
                        2;
                }
            }
            ...
            for (uint32_t y = tmpDim.getHeight(); y < side; y++) {
                for (uint32_t x = 0; x < side; x++) {
                    // we simply put it as a mix between the north and south border (they should be
                    // fairly similar because it is a toroidal world)
                    squareTmp[y * side + x] = (squareTmp[(0) * side + x] +
                                               squareTmp[(tmpDim.getHeight() - 1) * side + x]) /
                                              2;
                }
            }
            ...
            for (uint32_t y = 0; y < tmpDim.getHeight(); y++) {
                for (uint32_t x = 0; x < tmpDim.getWidth(); x++) {
                    deltas[y * tmpDim.getWidth() + x] =
                        squareTmp[y * side + x] - tmp[y * tmpDim.getWidth() + x];
                }
            }
            ...
            for (uint32_t y = 0; y < tmpDim.getHeight(); y++) {
                for (uint32_t x = 0; x < tmpDim.getWidth(); x++) {
                    ...
                    tmp[y * tmpDim.getWidth() + x] +=
                        (myDelta + specularWidthDelta + specularHeightDelta + oppositeDelta) / 4;
                }
            }
        ...
        msg = msg + Platec::to_string(tmpDim.getWidth()) + "x" +
              Platec::to_string(tmpDim.getHeight()) + " ";
        msg = msg + e.what();
