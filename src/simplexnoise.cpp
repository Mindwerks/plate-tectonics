...
float raw_noise_2d(const float x, const float y) {
    ...
    float F2 = (float)(0.5 * (sqrtf(3.0) - 1.0));
    // Hairy factor for 2D
    float s = (x + y) * F2;
    int i = fastfloor(x + s);
    int j = fastfloor(y + s);

    float G2 = (float)((3.0 - sqrtf(3.0)) / 6.0);
    float t = (i + j) * G2;
    ...
    float y1 = y0 - j1 + G2;
    float x2 = (float)(x0 - 1.0 + 2.0 * G2); // Offsets for last corner in (x,y) unskewed coords
    float y2 = (float)(y0 - 1.0 + 2.0 * G2);
...
float raw_noise_4d(const float x, const float y, const float z, const float w) {
    ...
    float F4 = (float)((sqrtf(5.0) - 1.0) / 4.0);
    float G4 = (float)((5.0 - sqrtf(5.0)) / 20.0);
